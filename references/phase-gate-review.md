...
**Check PGR-1-G: Directory-load pattern scan was performed**

This check detects the "invisible dependency" problem: source code that loads files by scanning
directories at runtime (e.g., `glob("references/*.md")`, `os.ReadDir("skills/")`), making those
files look dependency-free from static import analysis even though they are runtime data.

- **Step 1**: Verify that Step 3.5 was performed. Look for either:
  - A `dirload_summary` block in `asset-inventory.yaml` (generated by `--dirload-report`), OR
...
- **Step 2** (if Step 1 passes): Search source code files for at least these terms:
  ...
  ReadDir   WalkDir   readdirSync   Dir.glob   read_dir
  ```
  For each match found, verify the loaded directory's files have `runtime_dependency: true` in
  the inventory. If a directory-load pattern is present in source but its files are not tagged:

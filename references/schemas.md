...
Complete field definitions for all five persistence files, plus the derived
`ast-index.yaml` artifact (full_mode, see `references/ast-bridge.md`).
...
## ast-index.yaml (derived artifact — full_mode only)

Machine-generated by `scripts/ast_bridge.py index` (see `references/ast-bridge.md`).
**Not a persistence file**: regenerable from source at any time; never hand-edit.
If the source changes, regenerate and diff. Fallback extractors for unsupported
languages must produce this exact schema so `ast_bridge.py verify` keeps working.

| Field | Type | Description |
|-------|------|-------------|
| `meta.generated_at` | string | ISO 8601 timestamp of extraction run |
| `meta.script` | string | Path to the extraction script used |
| `meta.grammar` | string | tree-sitter grammar package + version |
| `meta.parse_failures` | list | Files that failed to parse (analyze manually) |
| `functions[].id` | string | Same id convention as ipo-registry (`file_stem::Class.name`) |
| `functions[].file` | string | Source file path |
| `functions[].start_line` / `end_line` | integer | Function definition node range |
| `functions[].signature` | string | Parameters + return annotation text |
| `functions[].parent_class` | string | Enclosing class name; empty for free functions |
| `functions[].branch_count` | integer | Count of conditional/loop/try AST nodes in body |
| `functions[].call_sites` | list | `{name, line}` for every call node in body |
| `functions[].literals` | list | `{value, type, line}` for every literal in body (excluding docstring) |
| `functions[].comments` | list | `{text, line}` for comments + docstring |
| `functions[].decorators` | list | Decorator names (may affect translation strategy) |

### `ast_bridge` block (per ipo-registry.yaml entry, when Stage 2 is used)

```yaml
ast_bridge:
  skeleton_generated: true
  skeleton_file: "internal/loop/loop.go"   # draft target file
  skeleton_lines: "34-89"                  # where the draft function landed
  todo_count_initial: 12                   # TODO(migrate) markers emitted
  todo_count_remaining: 0                  # must be 0 before translation_status: DONE (PGR-4-F)
```

## skeleton-map.yaml (derived artifact — full_mode only)

Written by `scripts/ast_bridge.py skeleton`. One entry per generated function:
`id`, `source_file`, `source_lines`, `skeleton_file`, `skeleton_lines`,
`todo_count_initial`, `todo_count_remaining`. The AI merges each entry into the
corresponding ipo-registry entry as its `ast_bridge` block, then keeps the registry
copy authoritative (the map is not updated as TODOs are resolved).
